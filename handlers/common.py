...
async def cmd_start(message: Message):
    ...
    # Проверяем, 8 марта ли сегодня
    today = datetime.now()
    is_march_8 = (today.month == 3 and today.day == 8)
    
    # Если баланс 0 - даем стартовый бонус
    if balance == 0:
        ...
        if is_march_8:
            welcome_text = (
                ...
                f"💰 Твой баланс: {balance + 888:.2f} LEDOGE\n\n"
        ...
        if is_march_8:
            # Даем праздничный бонус всем, у кого уже есть баланс
            update_balance(user_id, 'ledoge', 888, 'add')
            balance += 888
            welcome_text = (
                f"🌸🌷🌸 **С 8 МАРТА, {user.first_name}!** 🌸🌷🌸\n\n"
